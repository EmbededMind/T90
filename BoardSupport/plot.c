...
void DrawCursor(Point pixel, int flag)  //§¥???????§ç?
{
	int start_x, start_y;	
	Point point;
	char strTmp[15];
	...
	{	
		...
		if(start_x+infoWidth > SCREEN_WIDTH)    //???§¥?§ç???¡Î??¦¶
		{
...
Point GetItemPixel(Point itemPoint)    //???§Ô?§Ò???
{
	Point itemPixel;
...
static void FigureMotherShipPixel(Point center, int flag)    //?????§Ò???    center¨²????§¥??¦¶??§æ?
{
	Point screenCenter; //??¡Î?§¥??§Ò??§æ?
	if(!flag)
	{
		screenCenter.x = 215;
...
void DrawInvdShip(Point pixel, int course)  //course:?§Ó¨¨???¨¦  ??????
{
