...
int isCrossPointInLeft(BERTH *pBerth, Point pointa, Point pointb)
{
   if(pointa.y == pointb.y)
   {
      ...
      {
         if(pBerth->x_to_cross > MINNUM(pointa.x,pointb.x))
         {
            return 1;
         }
         return 0;         
   ...
   {
      if(pBerth->y_to_cross > MINNUM(pointa.x, pointb.x) && 
         pBerth->y_to_cross <= MAXNUM(pointa.x, pointb.x))
      {
         if(pBerth->x_to_cross >= pointa.x)
         {
   ...
   else if(pBerth->y_to_cross > MINNUM(pointa.x, pointb.x) && 
            pBerth->y_to_cross <= MAXNUM(pointa.x, pointb.x))
   { 
       if(pointa.y > pointb.y)
       {         
...
Bool isInPolygon(BERTH *pBerth)
{
   ...
   max = getAllPointYMax();
   min = getAllPointYMin();
   if(pBerth->y_to_cross > min && pBerth->y_to_cross < max)
   {
      for(i = 0; i < STUB_NUM_MAX_D; i++)
      {
         if(stubs[i].isValid)
