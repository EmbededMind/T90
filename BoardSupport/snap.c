...
//		{
//			pSnapLink = SimpBerthes[i].pBerth;
...
//	{
		while(pBully)
		{
			if(pBully->pBoatLink->mntState == MNTState_Triggered && pBully->pBoatLink->Boat.dist < dst)
			{
				pSnapLink = pBully->pBoatLink;
				dst = pBully->pBoatLink->Boat.dist;
			}
			pBully = pBully->pNext;
		}
//	}
